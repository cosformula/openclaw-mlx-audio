...
async function waitForServerHealthy(port) {
...
export default function register(api) {
    const rawConfig = api.config.plugins?.entries?.["mlx-audio"]?.config;
    ...
    api.registerTool({
        name: "mlx_audio_tts",
        description: "Generate speech audio locally using mlx-audio. Actions: generate (text→audio), status (server info).",
        parameters: {
            type: "object",
            properties: {
                action: {
        ...
        execute: async (_id, params) => {
            ...
            if (action === "generate") {
                ...
                if (!text)
                    return { content: [{ type: "text", text: JSON.stringify({ error: "text is required for generate action" }) }] };
